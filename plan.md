# Chat Workspace — แผนงานและสถานะโครงการ
...
### Repository ที่กำลังใช้งานใน workspace นี้

- Path: `D:\AI\chat-workspace`
- Branch: `master`
- Commit ล่าสุด ณ เวลาจัดทำแผน: `30104cb` — `feat: add main source files`
- Source, README, migration และ code editor ถูก commit แล้ว

### Repository ปลายทางอีกชุด

- Path: `D:\AI\zEXtream-Application-AI`
- Branch: `main`
- Remote: `https://github.com/kidsapondev/zEXtream-Application-AI.git`
- Commit: `0e58258` — `[ADD] Project`
- Local branch ตรงกับ `origin/main`

ทั้งสอง directory มี source เกือบเหมือนกัน ต่างหลัก ๆ ที่ `.gitignore`, `.idea/` และ root `Dockerfile`

### งานจัดระเบียบ Repository

- [ ] ตัดสินใจว่า directory ใดเป็น canonical working copy
- [ ] หากเลือก `zEXtream-Application-AI` ให้ย้ายการแก้ไขใหม่จาก `chat-workspace` ไปอย่างเป็นระบบ
- [ ] หากเลือก `chat-workspace` ให้ผูก remote และจัด branch naming ให้ตรงกับ repository ปลายทาง
- [ ] หลีกเลี่ยงแก้สอง directory พร้อมกัน เพราะเสี่ยงให้ source แยกจากกัน
...
- [ ] ทำ refresh-token rotation ให้เป็น atomic transaction
...
- [ ] ตรวจ `tokenHash` กับ refresh token ที่ได้รับจริง หรือถ้าไม่ใช้ให้ทบทวน design และลบ field ที่ทำให้เข้าใจผิด
...
- [ ] Concurrent refresh สอง request
- [ ] Logout แล้ว refresh ไม่ได้
- [ ] Hard reload แล้ว restore session ได้
- [ ] หลาย tab ใช้ refresh cookie เดียวกัน
...
- [ ] ตรวจ ownership ใน `chat:stop`
  - query message และ session owner ก่อน abort
  - ไม่ให้ user หนึ่งหยุด stream ของอีก user แม้ทราบ message ID
- [ ] Disconnect socket ตอน logout
- [ ] Reconnect/re-authenticate socket เมื่อ login เป็น user ใหม่
- [ ] ออกแบบการ refresh token สำหรับ socket reconnect เมื่อ access token หมดอายุ
...
- [ ] ครอบ streaming pipeline ด้วย `catch` ที่ finalize message ทุก failure path
- [ ] ย้าย `getProvider()`, registry setup และ parser processing เข้า error boundary เดียวกัน
- [ ] หาก provider ไม่มีอยู่ ให้ reject ก่อนสร้าง assistant message หรือ finalize เป็น `error`
...
- [ ] ทดสอบ end-to-end ด้วย prompt ที่ทำให้ Ollama ตอบ fenced code จริง
- [ ] ยืนยันว่า code ปรากฏใน Monaco แบบ progressive streaming
- [ ] ยืนยัน artifact row และ revision ถูกบันทึกถูกต้องใน PostgreSQL
- [ ] ยืนยัน reload session แล้วยังเห็นไฟล์ล่าสุด
- [ ] ยืนยัน user edit สร้าง revision และ diff ถูกต้อง
- [ ] ทดสอบ stop generation กลาง code fence
- [ ] ทดสอบ malformed/unterminated/multiple code fences

### P0/P1 — Defects ที่ต้องแก้

- [ ] นำ artifact revision ล่าสุดกลับเข้า AI context
...
- [ ] รวม user-edited artifact เข้า context ก่อน follow-up generation
- [ ] Debounce การ save จาก Monaco
  - ปัจจุบันทุก keystroke สร้าง WebSocket event และ database revision
  - กำหนด debounce เช่น 500–1000 ms หรือใช้ explicit Save
- [ ] ทำ revision creation ให้ concurrency-safe
...
- [ ] กรอง artifact socket events ด้วย current session ID
- [ ] ป้องกัน stale artifact HTTP response ตอนเปลี่ยน session
- [ ] แก้ parser ให้ยอมรับ closing fence ที่ EOF โดยไม่มี newline
- [ ] กำหนด behavior เมื่อ stream หยุดกลาง fence: save partial, discard หรือ mark incomplete
- [ ] Sanitize/normalize filename และป้องกัน path traversal หากอนาคตเขียนไฟล์ลง filesystem
- [ ] จำกัดขนาดและจำนวน artifacts ต่อ message/session
- [ ] ใช้ revision จริงจาก server ใน `artifact:stream:end` แทนสร้าง DTO revision `0` ฝั่ง client
- [ ] Dispose Monaco diff models เมื่อเปลี่ยน model/destroy เพื่อป้องกัน memory leak
...
- [~] Backend ESLint ยังไม่ผ่าน: พบ formatting, unsafe typing และ floating promise issues
- [ ] Test suite ยังไม่ครอบคลุม business logic หลัก

### Unit tests ที่ควรเพิ่มก่อน

- [ ] `ArtifactStreamParser` ทุก boundary condition
- [ ] `AuthService` token rotation/reuse/concurrency
- [ ] `ArtifactsService` revision selection/concurrency
...
- [ ] Auth endpoints กับ PostgreSQL จริงใน test container
- [ ] Session ownership ทุก REST endpoint
...
1. [ ] เลือก canonical repository และจัดการ `login.md`
2. [ ] แก้ socket disconnect/re-authentication ตอน logout/login
3. [ ] เพิ่ม ownership check ให้ `chat:stop`
4. [ ] ทำ streaming error boundary ให้ finalize message เสมอ
5. [ ] จำกัด provider ให้เหลือเฉพาะ Ollama จนกว่า provider อื่นพร้อม
6. [ ] ทำ refresh-token rotation ให้ atomic
7. [ ] เพิ่ม auth/WebSocket ownership integration tests
...
1. [ ] เพิ่ม artifacts ล่าสุดเข้า AI context
2. [ ] Debounce/explicit-save editor
3. [ ] ทำ revision concurrency-safe
4. [ ] แก้ parser EOF closing fence
5. [ ] ป้องกัน stale session/artifact state
6. [ ] ทำ Artifact E2E test matrix
7. [ ] เพิ่ม revision/diff UI ที่ใช้งานได้จริง
...
- [ ] เพิ่ม `SocketService.disconnect()` และเรียกจาก logout
- [ ] สร้าง socket ใหม่หลัง login/refresh identity change
- [ ] เพิ่ม message ownership query และตรวจใน `chat:stop`
- [ ] ย้าย provider lookup/stream/parser/finalize เข้า error boundary เดียว
- [ ] Reject `claude`/`openai` session creation ชั่วคราว หรือ implement provider registry validation
- [ ] เพิ่ม unit tests สำหรับ parser และ integration test สำหรับ WebSocket ownership
- [ ] เพิ่ม debounce ให้ Monaco edit
- [ ] ทำ artifact revision insert ให้ retry/lock เมื่อชน unique constraint
- [ ] ส่ง artifact content ล่าสุดเข้า AI context
- [ ] รัน Artifact E2E ด้วย Ollama จริง
...
- Docker development stack ของ `chat-workspace` ยังทำงานอยู่ ณ เวลาตรวจ: frontend, backend และ PostgreSQL
