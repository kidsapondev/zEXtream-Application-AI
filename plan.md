...
| Phase 7 | Security hardening และ production readiness | `[~]` | Helmet/CSP, CORS allowlist, rate limiting (REST+WS), audit log, structured logging, health checks, metrics, backup/restore, deployment hardening, file-based secrets, CI image scan, optional Sentry error reporting, license เสร็จหมด; account lockout ตัดสินใจไม่ทำแล้ว; manual test ทั้ง 2 รายการทำจริงแล้ว (server restart กลาง stream, migration rollback) — graceful-shutdown ยืนยันว่าทำงานถูกต้องจริงใน **prod** topology (`node dist/src/main.js` ตรง ๆ), พบเป็น dev-only gap (signal ไม่ลงถึง Nest process ผ่าน `pnpm start:dev`/`nest --watch`, priority ต่ำ) — ระหว่างทดสอบ prod เจอและแก้บั๊กจริง 2 จุด: prod image ไม่ set `NODE_ENV=production` เป็นค่า default ทำให้ crash-loop ตอน boot (`backend/Dockerfile`), และ compose overlay ไม่ pin `NODE_ENV` ทับ `.env` (`docker-compose.prod.yml`) — ดูรายละเอียดเต็มใน Phase 7 → Reliability |
