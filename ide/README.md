...
| `ctrl+p` | ค้นไฟล์แบบ fuzzy (`lcapp` เจอ `local_coder/app.py`) |
| `ctrl+f` | ค้น/แทนที่ในไฟล์ |
...
ในแผง review หลังโมเดลทำงานเสร็จ: `n`/`p` เลื่อนไฟล์, `a` รับ, `r` คืนค่าเดิม

## Review gate — กฎที่แอปนี้บังคับ

**ไม่มีอะไรที่โมเดลเขียนถือว่าจบจนกว่าคุณจะกดรับ** ทุกรอบที่สั่งงาน แอปจะเก็บสำเนาไฟล์ไว้ก่อน
พอโมเดลทำเสร็จจะแสดง diff ทีละไฟล์พร้อมปุ่ม Accept / Revert **แยกรายไฟล์** เพราะงานดีกับงานพัง
มักปนมาในรอบเดียวกัน

Revert เขียนสำเนาเดิมกลับ ไม่เรียก git — เพราะ workspace บ่อยครั้งไม่ใช่ repo และงานของโมเดล
มักปนอยู่กับงานที่คุณยังไม่ commit

**ข้อจำกัดที่ต้องรู้:** ไม่มีใครรู้ล่วงหน้าว่าโมเดลจะแตะไฟล์ไหน และถ้าไปอ่านทีหลังจะได้ผลงานของ
โมเดลเองเป็น "ก่อนแก้" (diff ว่าง revert ไร้ความหมาย) สำเนาจึงเก็บล่วงหน้าเฉพาะ **ไฟล์ที่คุณเปิดอยู่
และไฟล์ในโฟลเดอร์ที่คุณกางไว้ใน tree** ไฟล์นอกเหนือจากนั้นจะถูกรายงานตรง ๆ ว่ารีวิวไม่ได้
แทนที่จะเดา — เพราะเดาผิดแล้ว revert จะทับงานที่โมเดลไม่ได้แตะ

กางโฟลเดอร์ที่คาดว่าโมเดลจะทำงานไว้ก่อนสั่งงาน จะได้รีวิวได้ครบ
...
| `file_index.py` | index ไฟล์ + fuzzy match | **โมเดลในเครื่อง** (Claude แก้บั๊ก `max_files`) |
| `diff.py` | unified diff พร้อมเลขบรรทัดสองฝั่ง | Claude (โมเดลทำพลาดตรงเลขบรรทัด) |
| `review.py` | สำเนา/เทียบ/รับ/คืนค่า งานของโมเดล | Claude |
| `errors.py` | แปลง error เป็นข้อความที่บอกวิธีแก้ | Claude |
| `ui/editor_tabs.py` | แท็บหลายไฟล์ + dirty tracking | Claude |
| `ui/find_bar.py` | ค้น/แทนที่ในไฟล์ | Claude |
| `ui/file_finder.py` | overlay ค้นไฟล์ | Claude |
| `ui/search_panel.py` | ผลค้นทั้งโปรเจกต์ | Claude |
| `ui/review_panel.py` | หน้ารีวิว diff | Claude |
