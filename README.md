...
- สมัครสมาชิกและเข้าสู่ระบบด้วยอีเมล/รหัสผ่าน — บัญชีใหม่เริ่มที่สถานะ `guest` และใช้งานแชทไม่ได้จนกว่า Admin จะเปิดใช้งานให้ผ่าน Backoffice
...
- Backoffice สำหรับ Admin: จัดการ role/สถานะผู้ใช้, กำหนด permission แบบละเอียดต่อ admin แต่ละคน, ดู dashboard และ audit log ของการกระทำในระบบ (`/admin`)
...
├── docker-compose.deployment.yml # Optional: serve frontend from D:\AI\Deployment\chat.zextream.com (docs/deployment.md)
├── pnpm-workspace.yaml
└── package.json
```
...
| `BOOTSTRAP_ADMIN_EMAILS` | ไม่               | `admin@example.com`                      | Comma-separated list of emails ที่ระบบจะตั้งเป็น admin เต็มสิทธิ์เสมอ (idempotent ทุก startup/register) — สำหรับ dev/staging เท่านั้น ต้องล้างก่อนขึ้น production จริง |
...
Response มีข้อมูล user (รวม `role` และ `permissions`) และ access token พร้อมตั้ง refresh cookie — `role` ของบัญชีใหม่เริ่มที่ `guest` เสมอ เข้าสู่ระบบได้แต่เรียก resource endpoint อื่น ๆ (chat, artifacts, provider settings) จะได้ `403` จนกว่า admin จะเปลี่ยนเป็น `user`/`admin` ผ่าน `PATCH /api/admin/users/:id/role` (ดูหัวข้อ Backoffice ด้านล่าง) — frontend เด้งบัญชีสถานะนี้ไปหน้า `/account-pending` โดยอัตโนมัติ
...
คืนข้อมูล user ปัจจุบัน (รวม `role` และ `permissions`) โดยไม่คืน password hash — endpoint เดียวที่บัญชี `guest` เรียกได้

### Backoffice (Admin)

ทุก endpoint ในหมวดนี้ต้องเป็น `role: admin` และมี permission (`AdminPermission`) ที่กำหนดไว้เฉพาะ endpoint นั้น ๆ (ตรวจจาก database ทุก request ไม่ใช่จาก JWT) — ผู้ใช้ role อื่นได้ `403` เสมอ

#### `GET /api/admin/users?query=&limit=&offset=`

ต้องมี permission `users_view` — ค้นหา/แบ่งหน้ารายชื่อผู้ใช้ทั้งหมด

#### `GET /api/admin/users/:id`

ต้องมี permission `users_view` — คืนรายละเอียดผู้ใช้พร้อม permission ที่มี

#### `PATCH /api/admin/users/:id/status`

ต้องมี permission `users_manage_status` — `{ "isActive": false }` เปิด/ปิดการใช้งานบัญชี

#### `PATCH /api/admin/users/:id/role`

ต้องมี permission `users_manage_role` — `{ "role": "user" }` ใช้เปลี่ยน role รวมถึงเปิดใช้งานบัญชี `guest` (`guest` → `user`); demote จาก `admin` จะล้าง permission grants ทั้งหมดของบัญชีนั้นด้วย

#### `PUT /api/admin/users/:id/permissions`

ต้องมี permission `users_manage_permissions` — `{ "permissions": ["users_view", "dashboard_view"] }` แทนที่ permission ทั้งชุด ใช้ได้เฉพาะ target ที่เป็น role `admin` เท่านั้น

#### `GET /api/admin/dashboard`

ต้องมี permission `dashboard_view` — สรุปจำนวนผู้ใช้/สถานะ/provider ที่ configure ไว้

#### `GET /api/admin/audit-log?limit=&offset=`

ต้องมี permission `audit_log_view` — ประวัติการเปลี่ยน role/สถานะ/permission ที่ทำผ่าน backoffice

ทุก endpoint ที่แก้ status/role/permissions ปฏิเสธการแก้ไขบัญชีของตัวเอง (กัน admin ล็อกตัวเองออกจากระบบ) — ดูรายละเอียดออกแบบเต็มใน `plan.md` → Phase 8 และ `docs/threat-model.md`
